...
export default function NewInvoicePage() {
  ...
  const [dueDate, setDueDate] = useState(() => endOfNextMonth(new Date()));
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/invoices/issue", {
        ...
        body: JSON.stringify({ customerName, issueDate, dueDate, notes, lines }),
      });
      const body = await res.json();
      if (!res.ok) throw new Error(body.error || "作成に失敗しました");
      router.push(`/invoices/${body.id}/print`);
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-semibold">請求書を作成</h1>
        <p className="mt-1 text-sm text-slate-600">
          作成すると「売掛金 / 売上高・仮受消費税」の仕訳を自動で記帳し、印刷・PDF保存用の請求書を表示します。消費税は税率ごとに合計してから計算します(インボイス制度の方式)。
        </p>
      </div>
      ...
      <form onSubmit={submit} className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="sm:col-span-3 lg:col-span-1">
            <label className="mb-1 block text-xs text-slate-500">請求先(会社名・氏名)</label>
          ...
          <div>
            <label className="mb-1 block text-xs text-slate-500">お支払期限</label>
